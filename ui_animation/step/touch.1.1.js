...
(function ($) {
    ...
    Events.forEach(function (eventName) {
        $.fn[eventName] = function () {
            ...
            touch.start();
            return this.bind(eventName, arguments[0]);
        };
    });
    Touch = function () {
        var status, ts, tm, te;
    ...
    Touch.prototype.start = function () {
        var self = this;
        self.target.bind("touchstart", function (event) {
        ...
        self.target.bind("touchmove", function (event) {
            var temp = self.common(event);
            self.tm = {
                x: temp.t.pageX,
                y: temp.t.pageY,
        ...
        self.target.bind("touchend", function (event) {
            var temp = self.common(event);
    ...
    Touch.prototype.factory = function () {
        ...
        } else if (y>x && y > options.y) {
            if (t < 400) {
                if (this.te.y > 0) s = "swipeDown";
                else s = "swipeUp";
            } else s = "swipe";
        } else if (x>y && x > options.x) {
            if (t < 400) {
